...
class Soliloquy {
     ...
     * @var string
     */
    public $plugin_slug = 'soliloquy-v2';
...
function soliloquy_activation_hook( $network_wide ) {
    ...
    global $wp_version;
    if ( version_compare( $wp_version, '3.8', '<' ) && ! defined( 'SOLILOQUY_FORCE_ACTIVATION' ) ) {
        deactivate_plugins( plugin_basename( __FILE__ ) );
        wp_die( sprintf( __( 'Sorry, but your version of WordPress does not meet Soliloquy\'s required version of <strong>3.8</strong> to run properly. The plugin has been deactivated. <a href="%s">Click here to return to the Dashboard</a>.', 'soliloquy' ), get_admin_url() ) );
    }
